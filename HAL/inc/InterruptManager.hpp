...
public:
	...
	bool RegisterInterrupt(IRQn eIRQn, ISR pISR );
	static InterruptManager* GetInstance();
	void Enable_Interrupt(IRQn eIRQn);
	void Disable_Interrupt(IRQn eIRQn);
	void NVIC_Config(IRQn eIRQn, u8 Priority, u8 SubPriority);
private:
	InterruptManager(){};
	~InterruptManager(){};
